...
## Global settings
verbose        <- 1 #2 # [0,1,2,3] give some information while the script is running; 0 = silent
...
cpl_tag <- F # F: ocean-only, T: coupled
             # this concerns the fesom filename convention:
             # if cpl_tag == T
...
## Overwrite default experiment and mesh options with your options here
if (T) { # demo1
    runid <- "demo1"
    meshid <- "demomesh"
    cycl <- F
...
## Default depth options [m]
depths <- 0
integrate_depth <- F
# e.g.  depths = 0              -> sea surface
#       depths = c(23, 27.5)    -> average/integral between 
#       depths = c(42, "max")   -> average/integral between
#       depths = c(0, "MLD")    -> average/integral between
